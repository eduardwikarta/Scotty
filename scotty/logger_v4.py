...
def _validate_log_level(log_level: Union[str, int], log_location: str) -> int:
    ...
    _key = min(_valid_log_level_dict, key=lambda k: _valid_log_level_dict[k])
...
def ralt(log: logging.Logger, msg: str, f: Callable[..., Any], *args, **kwargs):
    result = f(*args, **kwargs)
    if msg: log.trace(msg, result) # type: ignore
    return result
